...
#define	 RESOURCE_TREE m_ResourceInfoDlg->m_TreeCtrl;
...
void CPEInfoDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_EDIT1, m_edit_VA);
	DDX_Control(pDX, IDC_EDIT2, m_edit_RVA);
	DDX_Control(pDX, IDC_EDIT3, m_edit_FOA);
}
...
BEGIN_MESSAGE_MAP(CPEInfoDlg, CDialogEx)
	...
	ON_WM_QUERYDRAGICON()
	ON_WM_DROPFILES()
	ON_NOTIFY(TVN_SELCHANGED, IDC_TREE1, &CPEInfoDlg::OnTvnSelchangedTree3)
	ON_WM_SIZE()
	ON_WM_MOVE()
...
BOOL CPEInfoDlg::OnInitDialog()
{
	...
	m_TableInfoDlg->Create(IDD_DIALOG1, this);
	m_ResourceInfoDlg = new ResourceInfoDlg;
	m_ResourceInfoDlg->Create(IDD_DIALOG2, this);
	m_CDisasmDlg = new CDisasmDlg;
	m_CDisasmDlg->Create(IDD_DIALOG3, this);
	m_HexEditorDlg = new CHexEditorDlg;
	m_HexEditorDlg->Create(IDD_DIALOG4, this);
	HICON hIcon[4];
	...
	DWORD ZZY = m_TreeCtrl.GetStyle();
	ZZY = m_TreeCtrl.GetExStyle();
...
void CPEInfoDlg::OnDropFiles(HDROP hDropInfo)
{
	...
	RootTitle += FileName;
	HTREEITEM hRoot = m_TreeCtrl.InsertItem(RootTitle, 0, 0, TVI_ROOT);
	...
	HTREEITEM hOpRoot = m_TreeCtrl.InsertItem(_T("可选头部"), 2, 2, hNtRoot);
	m_TreeCtrl.InsertItem(_T("数据目录"), 2, 2, hOpRoot);
	...
	FileAddress = (char*)malloc(FileSize);
	if (FileAddress == NULL)
	{
		CloseHandle(pFile);
	}
	ZeroMemory(FileAddress, FileSize);
	...
	m_TreeCtrl.InsertItem(_T("依赖性分析"), 1, 1, hRoot);
	m_TreeCtrl.InsertItem(_T("Hex编辑器"), 1, 1, hRoot);
	m_TreeCtrl.InsertItem(_T("快速反汇编"), 1, 1, hRoot);
	CloseHandle(pFile);
	MyExpandTree(hRoot);
...
void CPEInfoDlg::PrintFileInfo()
{
	...
	m_ListCtrl.ShowWindow(SW_SHOW);HideConvertGroup();
	...
	if (stFileData.nFileSizeLow > 0x40000000)
		_stprintf(FileSize, TEXT("%.2lfGB"), stFileData.nFileSizeLow / 1024.0 / 1024.0 / 1024.0);
	else if (stFileData.nFileSizeLow > 0x100000)
		_stprintf(FileSize, TEXT("%.2lfMB (%dbytes)"), stFileData.nFileSizeLow / 1024.0 / 1024.0, stFileData.nFileSizeLow);
	else
		_stprintf(FileSize, TEXT("%.2lfKB (%dbytes)"), stFileData.nFileSizeLow / 1024.0, stFileData.nFileSizeLow);
	m_ListCtrl.InsertItem(index, _T("文件名"));
	m_ListCtrl.SetItemText(index++, 1, FileName);
	m_ListCtrl.InsertItem(index, _T("完整路径"));
	m_ListCtrl.SetItemText(index++, 1, FileFullPath);
	...
	m_ListCtrl.SetItemText(index++, 1, FileSize);
	...
	m_ListCtrl.SetItemText(index++, 1, ChangeTime);
	m_ListCtrl.InsertItem(index, _T("访问时间"));
	m_ListCtrl.SetItemText(index++, 1, VisitTime);
	md5_encrypt(FileAddress, this->FileSize, md5);
	sha1_encrypt(FileAddress, this->FileSize, sha1);
	md4_encrypt(FileAddress, this->FileSize, md4);
	m_ListCtrl.InsertItem(index, _T("MD4"));
	m_ListCtrl.SetItemText(index++, 1, md4);
	m_ListCtrl.InsertItem(index, _T("MD5"));
	m_ListCtrl.SetItemText(index++, 1, md5);
	m_ListCtrl.InsertItem(index, _T("SHA-1"));
	m_ListCtrl.SetItemText(index++, 1, sha1);
	m_ListCtrl.InsertItem(index, _T("FileDescription"));
	m_ListCtrl.SetItemText(index++, 1, (LPCTSTR)FileDescription);
	m_ListCtrl.InsertItem(index, _T("CompanyName"));
	m_ListCtrl.SetItemText(index++, 1, (LPCTSTR)CompanyName);
	m_ListCtrl.InsertItem(index, _T("FileVersion"));
	m_ListCtrl.SetItemText(index++, 1, (LPCTSTR)FileVersion);
	m_ListCtrl.InsertItem(index, _T("LegalCopyright"));
	m_ListCtrl.SetItemText(index++, 1, (LPCTSTR)LegalCopyright);
	m_ListCtrl.InsertItem(index, _T("ProductName"));
	m_ListCtrl.SetItemText(index++, 1, (LPCTSTR)ProductName);
}
...
void CPEInfoDlg::PrintDosHeader()
{
	...
	m_ListCtrl.InsertItem(index, _T("e_magic"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pDosHeader->e_magic) - (DWORD)pDosHeader, 2));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pDosHeader->e_magic)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pDosHeader->e_magic, 4));
	m_ListCtrl.InsertItem(index, _T("e_cblp"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pDosHeader->e_cblp) - (DWORD)pDosHeader, 2));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pDosHeader->e_cblp)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pDosHeader->e_cblp, 4));
	m_ListCtrl.InsertItem(index, _T("e_cp"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pDosHeader->e_cp) - (DWORD)pDosHeader, 2));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pDosHeader->e_cp)));
	m_ListCtrl.SetItemText(index, 3, Int2CString(pDosHeader->e_cp, 4));
	m_ListCtrl.InsertItem(index, _T("e_crlc"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pDosHeader->e_crlc) - (DWORD)pDosHeader, 2));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pDosHeader->e_crlc)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pDosHeader->e_crlc, 4));
	m_ListCtrl.InsertItem(index, _T("e_cparhdr"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pDosHeader->e_cparhdr) - (DWORD)pDosHeader, 2));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pDosHeader->e_cparhdr)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pDosHeader->e_cparhdr, 4));
	m_ListCtrl.InsertItem(index, _T("e_minalloc"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pDosHeader->e_minalloc) - (DWORD)pDosHeader, 2));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pDosHeader->e_minalloc)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pDosHeader->e_minalloc, 4));
	m_ListCtrl.InsertItem(index, _T("e_maxalloc"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pDosHeader->e_maxalloc) - (DWORD)pDosHeader, 2));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pDosHeader->e_maxalloc)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pDosHeader->e_maxalloc, 4));
	m_ListCtrl.InsertItem(index, _T("e_ss"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pDosHeader->e_ss) - (DWORD)pDosHeader, 2));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pDosHeader->e_ss)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pDosHeader->e_ss, 4));
	m_ListCtrl.InsertItem(index, _T("e_sp"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pDosHeader->e_sp) - (DWORD)pDosHeader, 2));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pDosHeader->e_sp)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pDosHeader->e_sp, 4));
	m_ListCtrl.InsertItem(index, _T("e_csum"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pDosHeader->e_csum) - (DWORD)pDosHeader, 2));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pDosHeader->e_csum)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pDosHeader->e_csum, 4));
	m_ListCtrl.InsertItem(index, _T("e_ip"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pDosHeader->e_ip) - (DWORD)pDosHeader, 2));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pDosHeader->e_ip)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pDosHeader->e_ip, 4));
	m_ListCtrl.InsertItem(index, _T("e_cs"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pDosHeader->e_cs) - (DWORD)pDosHeader, 2));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pDosHeader->e_cs)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pDosHeader->e_cs, 4));
	m_ListCtrl.InsertItem(index, _T("e_lfarlc"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pDosHeader->e_lfarlc) - (DWORD)pDosHeader, 2));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pDosHeader->e_lfarlc)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pDosHeader->e_lfarlc, 4));
	...
	m_ListCtrl.InsertItem(index, _T("e_oemid"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pDosHeader->e_oemid) - (DWORD)pDosHeader, 2));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pDosHeader->e_oemid)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pDosHeader->e_oemid, 4));
	m_ListCtrl.InsertItem(index, _T("e_oeminfo"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pDosHeader->e_oeminfo) - (DWORD)pDosHeader, 2));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pDosHeader->e_oeminfo)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pDosHeader->e_oeminfo, 4));
	...
	m_ListCtrl.InsertItem(index, _T("e_lfanew"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pDosHeader->e_lfanew) - (DWORD)pDosHeader, 2));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pDosHeader->e_lfanew)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pDosHeader->e_lfanew, 4));
}
...
void CPEInfoDlg::PrintNtHeader()
{
	...
	m_ListCtrl.InsertItem(0, _T("Signature"));
	m_ListCtrl.SetItemText(0, 1, Int2CString((DWORD) & (pNtHeader->Signature) - (DWORD)FileAddress));
	m_ListCtrl.SetItemText(0, 2, GetVariableSize(sizeof(pNtHeader->Signature)));
	m_ListCtrl.SetItemText(0, 3, Int2CString(pNtHeader->Signature));
	int index = 0;
}
...
CString CPEInfoDlg::Int2CString(int data, int length)
{
	CString format;
...
CString CPEInfoDlg::Int2CString(int data)
{
	CString ret;
...
void CPEInfoDlg::OnTvnSelchangedTree3(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	CString ItemText = m_TreeCtrl.GetItemText(pNMTreeView->itemNew.hItem);
...
void CPEInfoDlg::PrintFileHeader()
{
	...
	int index = 0;
	m_ListCtrl.InsertItem(index, _T("Machine"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pFileHeader->Machine) - (DWORD)FileAddress));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pFileHeader->Machine)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pFileHeader->Machine, 8));
	m_ListCtrl.InsertItem(index, _T("NumberOfSections"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pFileHeader->NumberOfSections) - (DWORD)FileAddress));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pFileHeader->NumberOfSections)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pFileHeader->NumberOfSections, 8));
	m_ListCtrl.InsertItem(index, _T("TimeDateStamp"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pFileHeader->TimeDateStamp) - (DWORD)FileAddress));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pFileHeader->TimeDateStamp)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pFileHeader->TimeDateStamp, 8));
	m_ListCtrl.InsertItem(index, _T("PointerToSymbolTable"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pFileHeader->PointerToSymbolTable) - (DWORD)FileAddress));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pFileHeader->PointerToSymbolTable)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pFileHeader->PointerToSymbolTable, 8));
	m_ListCtrl.InsertItem(index, _T("NumberOfSymbols"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pFileHeader->NumberOfSymbols) - (DWORD)FileAddress));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pFileHeader->NumberOfSymbols)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pFileHeader->NumberOfSymbols, 8));
	m_ListCtrl.InsertItem(index, _T("SizeOfOptionalHeader"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pFileHeader->SizeOfOptionalHeader) - (DWORD)FileAddress));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pFileHeader->SizeOfOptionalHeader)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pFileHeader->SizeOfOptionalHeader, 8));
	m_ListCtrl.InsertItem(index, _T("Characteristics"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pFileHeader->Characteristics) - (DWORD)FileAddress));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pFileHeader->Characteristics)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pFileHeader->Characteristics, 8));
}
...
void CPEInfoDlg::PrintOptionalHeader()
{
	...
	ClearListCtrl();
	m_ListCtrl.InsertColumn(0, _T("成员"), LVCFMT_LEFT, 180);
	m_ListCtrl.InsertColumn(1, _T("偏移量"), LVCFMT_LEFT, 100);
	m_ListCtrl.InsertColumn(2, _T("大小"), LVCFMT_LEFT, 100);
	m_ListCtrl.InsertColumn(3, _T("值"), LVCFMT_LEFT, 100);

	int index = 0;
	m_ListCtrl.InsertItem(index, _T("Magic"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pOptionalHeader->Magic) - (DWORD)FileAddress, 3));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pOptionalHeader->Magic)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pOptionalHeader->Magic, 8));
	m_ListCtrl.InsertItem(index, _T("MajorLinkerVersion"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pOptionalHeader->MajorLinkerVersion) - (DWORD)FileAddress, 3));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pOptionalHeader->MajorLinkerVersion)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pOptionalHeader->MajorLinkerVersion, 8));
	m_ListCtrl.InsertItem(index, _T("MinorLinkerVersion"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pOptionalHeader->MinorLinkerVersion) - (DWORD)FileAddress, 3));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pOptionalHeader->MinorLinkerVersion)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pOptionalHeader->MinorLinkerVersion, 8));
	m_ListCtrl.InsertItem(index, _T("SizeOfCode"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pOptionalHeader->SizeOfCode) - (DWORD)FileAddress, 3));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pOptionalHeader->SizeOfCode)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pOptionalHeader->SizeOfCode, 8));
	m_ListCtrl.InsertItem(index, _T("SizeOfInitializedData"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pOptionalHeader->SizeOfInitializedData) - (DWORD)FileAddress, 3));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pOptionalHeader->SizeOfInitializedData)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pOptionalHeader->SizeOfInitializedData, 8));
	m_ListCtrl.InsertItem(index, _T("SizeOfUninitializedData"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pOptionalHeader->SizeOfUninitializedData) - (DWORD)FileAddress, 3));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pOptionalHeader->SizeOfUninitializedData)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pOptionalHeader->SizeOfUninitializedData, 8));
	m_ListCtrl.InsertItem(index, _T("AddressOfEntryPoint"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pOptionalHeader->AddressOfEntryPoint) - (DWORD)FileAddress, 3));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pOptionalHeader->AddressOfEntryPoint)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pOptionalHeader->AddressOfEntryPoint, 8));
	m_ListCtrl.InsertItem(index, _T("BaseOfCode"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pOptionalHeader->BaseOfCode) - (DWORD)FileAddress, 3));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pOptionalHeader->BaseOfCode)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pOptionalHeader->BaseOfCode, 8));
	m_ListCtrl.InsertItem(index, _T("BaseOfData"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pOptionalHeader->BaseOfData) - (DWORD)FileAddress, 3));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pOptionalHeader->BaseOfData)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pOptionalHeader->BaseOfData, 8));
	m_ListCtrl.InsertItem(index, _T("ImageBase"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pOptionalHeader->ImageBase) - (DWORD)FileAddress, 3));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pOptionalHeader->ImageBase)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pOptionalHeader->ImageBase, 8));
	m_ListCtrl.InsertItem(index, _T("SectionAlignment"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pOptionalHeader->SectionAlignment) - (DWORD)FileAddress, 3));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pOptionalHeader->SectionAlignment)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pOptionalHeader->SectionAlignment, 8));
	m_ListCtrl.InsertItem(index, _T("FileAlignment"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pOptionalHeader->FileAlignment) - (DWORD)FileAddress, 3));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pOptionalHeader->FileAlignment)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pOptionalHeader->FileAlignment, 8));
	m_ListCtrl.InsertItem(index, _T("MajorOperatingSystemVersion"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pOptionalHeader->MajorOperatingSystemVersion) - (DWORD)FileAddress, 3));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pOptionalHeader->MajorOperatingSystemVersion)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pOptionalHeader->MajorOperatingSystemVersion, 8));
	m_ListCtrl.InsertItem(index, _T("MinorOperatingSystemVersion"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pOptionalHeader->MinorOperatingSystemVersion) - (DWORD)FileAddress, 3));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pOptionalHeader->MinorOperatingSystemVersion)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pOptionalHeader->MinorOperatingSystemVersion, 8));
	m_ListCtrl.InsertItem(index, _T("Win32VersionValue"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pOptionalHeader->Win32VersionValue) - (DWORD)FileAddress, 3));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pOptionalHeader->Win32VersionValue)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pOptionalHeader->Win32VersionValue, 8));
	m_ListCtrl.InsertItem(index, _T("SizeOfImage"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pOptionalHeader->SizeOfImage) - (DWORD)FileAddress, 3));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pOptionalHeader->SizeOfImage)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pOptionalHeader->SizeOfImage, 8));
	m_ListCtrl.InsertItem(index, _T("SizeOfHeaders"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pOptionalHeader->SizeOfHeaders) - (DWORD)FileAddress, 3));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pOptionalHeader->SizeOfHeaders)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pOptionalHeader->SizeOfHeaders, 8));
	m_ListCtrl.InsertItem(index, _T("CheckSum"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pOptionalHeader->CheckSum) - (DWORD)FileAddress, 3));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pOptionalHeader->CheckSum)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pOptionalHeader->CheckSum, 8));
	m_ListCtrl.InsertItem(index, _T("Subsystem"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pOptionalHeader->Subsystem) - (DWORD)FileAddress, 3));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pOptionalHeader->Subsystem)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pOptionalHeader->Subsystem, 8));
	m_ListCtrl.InsertItem(index, _T("DllCharacteristics"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pOptionalHeader->DllCharacteristics) - (DWORD)FileAddress, 3));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pOptionalHeader->DllCharacteristics)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pOptionalHeader->DllCharacteristics, 8));
	m_ListCtrl.InsertItem(index, _T("SizeOfStackReserve"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pOptionalHeader->SizeOfStackReserve) - (DWORD)FileAddress, 3));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pOptionalHeader->SizeOfStackReserve)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pOptionalHeader->SizeOfStackReserve, 8));
	m_ListCtrl.InsertItem(index, _T("SizeOfStackCommit"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pOptionalHeader->SizeOfStackCommit) - (DWORD)FileAddress, 3));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pOptionalHeader->SizeOfStackCommit)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pOptionalHeader->SizeOfStackCommit, 8));
	m_ListCtrl.InsertItem(index, _T("SizeOfHeapReserve"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pOptionalHeader->SizeOfHeapReserve) - (DWORD)FileAddress, 3));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pOptionalHeader->SizeOfHeapReserve)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pOptionalHeader->SizeOfHeapReserve, 8));
	m_ListCtrl.InsertItem(index, _T("SizeOfHeapCommit"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pOptionalHeader->SizeOfHeapCommit) - (DWORD)FileAddress, 3));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pOptionalHeader->SizeOfHeapCommit)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pOptionalHeader->SizeOfHeapCommit, 8));
	m_ListCtrl.InsertItem(index, _T("LoaderFlags"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pOptionalHeader->LoaderFlags) - (DWORD)FileAddress, 3));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pOptionalHeader->LoaderFlags)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pOptionalHeader->LoaderFlags, 8));
	m_ListCtrl.InsertItem(index, _T("NumberOfRvaAndSizes"));
	m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (pOptionalHeader->NumberOfRvaAndSizes) - (DWORD)FileAddress, 3));
	m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pOptionalHeader->NumberOfRvaAndSizes)));
	m_ListCtrl.SetItemText(index++, 3, Int2CString(pOptionalHeader->NumberOfRvaAndSizes, 8));
...
void CPEInfoDlg::PrintSectionHeader()
{
	...
	m_ListCtrl.InsertColumn(3, _T("Raw大小"), LVCFMT_LEFT, 100);
	m_ListCtrl.InsertColumn(4, _T("Raw地址"), LVCFMT_LEFT, 100);
...
#endif
		m_ListCtrl.InsertItem(i + 1, name);
		m_ListCtrl.SetItemText(i + 1, 1, Int2CString(pImageSectionHeader[i].Misc.VirtualSize, 8));
		m_ListCtrl.SetItemText(i + 1, 2, Int2CString(pImageSectionHeader[i].VirtualAddress, 8));
		m_ListCtrl.SetItemText(i + 1, 3, Int2CString(pImageSectionHeader[i].SizeOfRawData, 8));
		m_ListCtrl.SetItemText(i + 1, 4, Int2CString(pImageSectionHeader[i].PointerToRawData, 8));
	}
}
...
void CPEInfoDlg::PrintDataDirectory()
{
	...
	ClearListCtrl();
	m_ListCtrl.InsertColumn(0, _T("成员"), LVCFMT_LEFT, 220);
	m_ListCtrl.InsertColumn(1, _T("偏移量"), LVCFMT_LEFT, 100);
	m_ListCtrl.InsertColumn(2, _T("大小"), LVCFMT_LEFT, 100);
	m_ListCtrl.InsertColumn(3, _T("值"), LVCFMT_LEFT, 100);
	m_ListCtrl.InsertColumn(4, _T("段"), LVCFMT_LEFT, 100);
	for (int i = 0; i < 15; i++)
	{
		DWORD ExportDirectory_RVA = pImageOptionalHeader->DataDirectory[i].VirtualAddress;
		int index = 2 * i;
		m_ListCtrl.InsertItem(index, DirectoryName[index]);
		m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD)&pImageOptionalHeader->DataDirectory[i].VirtualAddress - (DWORD)FileAddress, 8));
		m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pImageOptionalHeader->DataDirectory[i].VirtualAddress, 2)));
		m_ListCtrl.SetItemText(index, 3, Int2CString(pImageOptionalHeader->DataDirectory[i].VirtualAddress, 8));
		TCHAR SectionName[9] = { 0 };
		GetSectionName(pImageOptionalHeader->DataDirectory[i].VirtualAddress, SectionName);
		m_ListCtrl.SetItemText(index, 4, SectionName);
		index++;
		m_ListCtrl.InsertItem(index, DirectoryName[index]);
		m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD)&pImageOptionalHeader->DataDirectory[i].Size - (DWORD)FileAddress, 8));
		m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(pImageOptionalHeader->DataDirectory[i].Size, 2)));
		m_ListCtrl.SetItemText(index, 3, Int2CString(pImageOptionalHeader->DataDirectory[i].Size, 8));
		m_ListCtrl.SetItemText(index, 4, _T(""));
	}
...
void CPEInfoDlg::PrintImportTable()
{
	...
	m_ListCtrl.ShowWindow(SW_HIDE);HideConvertGroup();
	m_ResourceInfoDlg->ShowWindow(SW_HIDE);
	m_CDisasmDlg->ShowWindow(SW_HIDE);
...
void CPEInfoDlg::PrintResourceTable()
{
	...
	m_CDisasmDlg->ShowWindow(SW_HIDE);
	m_HexEditorDlg->ShowWindow(SW_HIDE);
	m_ResourceInfoDlg->m_TreeCtrl.DeleteAllItems();
	PIMAGE_DOS_HEADER pImageDosHeader = (PIMAGE_DOS_HEADER)FileAddress;
	PIMAGE_FILE_HEADER pImageFileHeader = (PIMAGE_FILE_HEADER)((DWORD)pImageDosHeader + pImageDosHeader->e_lfanew + sizeof(DWORD));
	PIMAGE_OPTIONAL_HEADER pImageOptionalHeader = (PIMAGE_OPTIONAL_HEADER)((DWORD)pImageFileHeader + sizeof(IMAGE_FILE_HEADER));
	...
	HTREEITEM hRoot = m_ResourceInfoDlg->m_TreeCtrl.InsertItem(_T("资源目录"), 0, 0, TVI_ROOT);
	PRESOURCE_DATA ResourceData = new RESOURCE_DATA;
	ResourceData->Type = 1;
	ResourceData->Address = (DWORD)pResRoot;
	m_ResourceInfoDlg->m_TreeCtrl.SetItemData(hRoot, (DWORD_PTR)ResourceData);
	...
	for (DWORD i = 0; i < dwCountOfEntry1; ++i) {
		...
		HTREEITEM tmpItem = m_ResourceInfoDlg->m_TreeCtrl.InsertItem(tmpStr, hRoot);
		PRESOURCE_DATA ResourceData1 = new RESOURCE_DATA;
		ResourceData1->Type = 2;
		ResourceData1->Address = (DWORD)&pDirEntry1[i];
		m_ResourceInfoDlg->m_TreeCtrl.SetItemData(tmpItem, (DWORD_PTR)ResourceData1);
		...
		tmpItem = m_ResourceInfoDlg->m_TreeCtrl.InsertItem(_T("资源目录"), tmpItem);
		PRESOURCE_DATA ResourceData2 = new RESOURCE_DATA;
		ResourceData2->Type = 1;
		ResourceData2->Address = (DWORD)pDir2;
		m_ResourceInfoDlg->m_TreeCtrl.SetItemData(tmpItem, (DWORD_PTR)ResourceData2);
		...
		for (DWORD i = 0; i < dwCountOfEntry2; ++i) {
			...
			HTREEITEM tmpItem3 = m_ResourceInfoDlg->m_TreeCtrl.InsertItem(tmpStr2, tmpItem2);
			PRESOURCE_DATA ResourceData5 = new RESOURCE_DATA;
			ResourceData5->Type = 2;
			ResourceData5->Address = (DWORD)&pDirEntry3[i];
			m_ResourceInfoDlg->m_TreeCtrl.SetItemData(tmpItem3, (DWORD_PTR)ResourceData5);
			tmpItem3 = m_ResourceInfoDlg->m_TreeCtrl.InsertItem(_T("资源数据项"), tmpItem3);
			PRESOURCE_DATA ResourceData6 = new RESOURCE_DATA;
			ResourceData6->Type = 3;
			ResourceData6->Address = (DWORD)pDataEntry;
			m_ResourceInfoDlg->m_TreeCtrl.SetItemData(tmpItem3, (DWORD_PTR)ResourceData6);
		}
	}
	m_ResourceInfoDlg->m_TreeCtrl.Expand(hRoot, TVE_EXPAND);
}
...
void CPEInfoDlg::PrintResourceItem(NMHDR* pNMHDR, LRESULT* pResult)
{
	LPNMTREEVIEW pNMTreeView = reinterpret_cast<LPNMTREEVIEW>(pNMHDR);
	HTREEITEM hitem = pNMTreeView->itemNew.hItem;
	PRESOURCE_DATA ResourceData = (PRESOURCE_DATA)m_TreeCtrl.GetItemData(pNMTreeView->itemNew.hItem);
	if (ResourceData == NULL)
	{
		return;
	}
	m_ResourceInfoDlg->m_ListCtrl.DeleteAllItems();
	int index = 0;
	...
	{
	case 1:
		m_ResourceInfoDlg->m_ListCtrl.InsertItem(index, _T("Characteristics"));
		m_ResourceInfoDlg->m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (ResourceDirectory->Characteristics) - (DWORD)FileAddress, 8));
		m_ResourceInfoDlg->m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(ResourceDirectory->Characteristics)));
		m_ResourceInfoDlg->m_ListCtrl.SetItemText(index++, 3, Int2CString(ResourceDirectory->Characteristics, 2 * sizeof(ResourceDirectory->Characteristics)));
		m_ResourceInfoDlg->m_ListCtrl.InsertItem(index, _T("TimeDateStamp"));
		m_ResourceInfoDlg->m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (ResourceDirectory->TimeDateStamp) - (DWORD)FileAddress, 8));
		m_ResourceInfoDlg->m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(ResourceDirectory->TimeDateStamp)));
		m_ResourceInfoDlg->m_ListCtrl.SetItemText(index++, 3, Int2CString(ResourceDirectory->TimeDateStamp, 2 * sizeof(ResourceDirectory->TimeDateStamp)));
		m_ResourceInfoDlg->m_ListCtrl.InsertItem(index, _T("MajorVersion"));
		m_ResourceInfoDlg->m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (ResourceDirectory->MajorVersion) - (DWORD)FileAddress, 8));
		m_ResourceInfoDlg->m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(ResourceDirectory->MajorVersion)));
		m_ResourceInfoDlg->m_ListCtrl.SetItemText(index++, 3, Int2CString(ResourceDirectory->MajorVersion, 2 * sizeof(ResourceDirectory->MajorVersion)));
		m_ResourceInfoDlg->m_ListCtrl.InsertItem(index, _T("MinorVersion"));
		m_ResourceInfoDlg->m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (ResourceDirectory->MinorVersion) - (DWORD)FileAddress, 8));
		m_ResourceInfoDlg->m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(ResourceDirectory->MinorVersion)));
		m_ResourceInfoDlg->m_ListCtrl.SetItemText(index++, 3, Int2CString(ResourceDirectory->MinorVersion, 2 * sizeof(ResourceDirectory->MinorVersion)));
		m_ResourceInfoDlg->m_ListCtrl.InsertItem(index, _T("NumberOfNamedEntries"));
		m_ResourceInfoDlg->m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (ResourceDirectory->NumberOfNamedEntries) - (DWORD)FileAddress, 8));
		m_ResourceInfoDlg->m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(ResourceDirectory->NumberOfNamedEntries)));
		m_ResourceInfoDlg->m_ListCtrl.SetItemText(index++, 3, Int2CString(ResourceDirectory->NumberOfNamedEntries, 2 * sizeof(ResourceDirectory->NumberOfNamedEntries)));
		m_ResourceInfoDlg->m_ListCtrl.InsertItem(index, _T("NumberOfIdEntries"));
		m_ResourceInfoDlg->m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (ResourceDirectory->NumberOfIdEntries) - (DWORD)FileAddress, 8));
		m_ResourceInfoDlg->m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(ResourceDirectory->NumberOfIdEntries)));
		m_ResourceInfoDlg->m_ListCtrl.SetItemText(index++, 3, Int2CString(ResourceDirectory->NumberOfIdEntries, 2 * sizeof(ResourceDirectory->NumberOfIdEntries)));
		break;
	case 2:
		m_ResourceInfoDlg->m_ListCtrl.InsertItem(index, _T("Name"));
		m_ResourceInfoDlg->m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (ResourceDirectoryEntry->Name) - (DWORD)FileAddress, 8));
		m_ResourceInfoDlg->m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(ResourceDirectoryEntry->Name)));
		m_ResourceInfoDlg->m_ListCtrl.SetItemText(index++, 3, Int2CString(ResourceDirectoryEntry->Name, 2 * sizeof(ResourceDirectoryEntry->Name)));
		m_ResourceInfoDlg->m_ListCtrl.InsertItem(index, _T("OffsetToData"));
		m_ResourceInfoDlg->m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (ResourceDirectoryEntry->OffsetToData) - (DWORD)FileAddress, 8));
		m_ResourceInfoDlg->m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(ResourceDirectoryEntry->OffsetToData)));
		m_ResourceInfoDlg->m_ListCtrl.SetItemText(index++, 3, Int2CString(ResourceDirectoryEntry->OffsetToData, 2 * sizeof(ResourceDirectoryEntry->OffsetToData)));
		break;
	case 3:
		m_ResourceInfoDlg->m_ListCtrl.InsertItem(index, _T("OffsetToData"));
		m_ResourceInfoDlg->m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (ResourceDataEntry->OffsetToData) - (DWORD)FileAddress, 8));
		m_ResourceInfoDlg->m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(ResourceDataEntry->OffsetToData)));
		m_ResourceInfoDlg->m_ListCtrl.SetItemText(index++, 3, Int2CString(ResourceDataEntry->OffsetToData, 2 * sizeof(ResourceDataEntry->OffsetToData)));
		m_ResourceInfoDlg->m_ListCtrl.InsertItem(index, _T("Size"));
		m_ResourceInfoDlg->m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (ResourceDataEntry->Size) - (DWORD)FileAddress, 8));
		m_ResourceInfoDlg->m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(ResourceDataEntry->Size)));
		m_ResourceInfoDlg->m_ListCtrl.SetItemText(index++, 3, Int2CString(ResourceDataEntry->Size, 2 * sizeof(ResourceDataEntry->Size)));
		m_ResourceInfoDlg->m_ListCtrl.InsertItem(index, _T("CodePage"));
		m_ResourceInfoDlg->m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (ResourceDataEntry->CodePage) - (DWORD)FileAddress, 8));
		m_ResourceInfoDlg->m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(ResourceDataEntry->CodePage)));
		m_ResourceInfoDlg->m_ListCtrl.SetItemText(index++, 3, Int2CString(ResourceDataEntry->CodePage, 2 * sizeof(ResourceDataEntry->CodePage)));
		m_ResourceInfoDlg->m_ListCtrl.InsertItem(index, _T("Reserved"));
		m_ResourceInfoDlg->m_ListCtrl.SetItemText(index, 1, Int2CString((DWORD) & (ResourceDataEntry->Reserved) - (DWORD)FileAddress, 8));
		m_ResourceInfoDlg->m_ListCtrl.SetItemText(index, 2, GetVariableSize(sizeof(ResourceDataEntry->Reserved)));
		m_ResourceInfoDlg->m_ListCtrl.SetItemText(index++, 3, Int2CString(ResourceDataEntry->Reserved, 2 * sizeof(ResourceDataEntry->Reserved)));
		break;
	}
}
...
void CPEInfoDlg::PrintExportTable()
{
	...
	DWORD AddressOfFunctions_FOA, AddressOfNames_FOA, AddressOfNameOrdinals_FOA;
	...
	for (int i = 0; i < ExportDirectoryInfoVec.size(); i++)
	{
...
void CPEInfoDlg::PrintHexEditor()
{
	...
	m_HexEditorDlg->m_HexEditor.SetData(FileSize, (BYTE*)FileAddress);
}
...
void CPEInfoDlg::OnLvnItemchangedList2(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	CString ItemText = m_TreeCtrl.GetItemText(TreeItem);
	POSITION ps;
	int nIndex;
...
{
	m_CDisasmDlg->m_ListCtrl.DeleteAllItems();
	DISASM disAsm = { 0 };
	...
	while (nCount < nOpcodeSize)
	{
		...
		m_CDisasmDlg->m_ListCtrl.InsertItem(index, Int2CString(uAddr));
		...
		m_CDisasmDlg->m_ListCtrl.SetItemText(index, 1, OPCODE);
		m_CDisasmDlg->m_ListCtrl.SetItemText(index++, 2, Instruct);
