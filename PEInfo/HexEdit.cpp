...
CHexEdit::CHexEdit()
{
	...
	m_bReadOnly = FALSE;
	m_nBytesPerRow = 0x10;
	m_nLimit = 0xFFFFFFF;	// Default max bytes
...
{
	nCount += ((GetClientRows() - 1) * m_nBytesPerRow);
	ASSERT(nCount >= 0);
	SetCurrPos(nCount);
	return m_nPosition;
}
...
void CHexEdit::SetDisplayMetrics()
{
	...
	m_ColMetrics[COL_OFFSET].right = m_ColMetrics[COL_OFFSET].left + (m_CharSize.cx * 8);
...
void CHexEdit::SetScroll(UINT nTopPosition)
{
	if (nTopPosition < 0)
		nTopPosition = 0;
...
void CHexEdit::OnPaint()
{
	...
	if (HasSelection())
	{
		...
		while (cyRow < rect.bottom && nPosition < m_nLength)
		{
			...
			dc.ExtTextOut(m_ColMetrics[COL_OFFSET].left, cyRow, 0, NULL, szBuffer, 8, NULL);
...
void CHexEdit::GetOffsetString(TCHAR* sBuffer, int nOffset)
{
	...
	*sBuffer++ = szHexDigits[(nOffset >> (4 * 7)) & 0xf];
	*sBuffer++ = szHexDigits[(nOffset >> (4 * 6)) & 0xf];
	*sBuffer++ = szHexDigits[(nOffset >> (4 * 5)) & 0xf];
	*sBuffer++ = szHexDigits[(nOffset >> (4 * 4)) & 0xf];
...
void CHexEdit::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* /*pScrollBar*/)
{
	...
	case SB_THUMBPOSITION:
	{
		...
		int uScrollPos = info.nTrackPos;
		UINT i = uScrollPos * m_nBytesPerRow;
		if (m_nTopPosition != i)
		{
			SetScroll(i);
			UpdateScrollbar();
